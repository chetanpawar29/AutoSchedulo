<!DOCTYPE html>
<%@page import="com.scheduling.model.Route"%>
		...
		<div class="table-responsive">
			<table class="table" style="text-align: center;font-size: 5rem; border: 2px solid; border-bottom: 2px solid;">
				...
				<tr>
					...
					<td><%=r.getTime()%></td>
					<td><%=r.getRstatus()%></td>
					<td><a href="updateR?rid=<%=r.getRid()%>&rsource=<%=r.getRsource()%>&rdest=<%=r.getRdest() %>&rmid=<%=r.getRmid()%>&time=<%=r.getTime() %>&rstatus=<%=r.getRstatus() %>"><span class="badge badge-info">Update</span></a></td>
					<td><a href="deleteR?rid=<%=r.getRid()%>"><span class="badge badge-danger">Delete</span></a></td>
					</tr>
				<% } %>
				</tbody>
			</table>
