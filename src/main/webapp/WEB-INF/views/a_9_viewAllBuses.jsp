			...
			<table class="table" style="text-align: center;font-size: 5rem; border: 2px solid; border-bottom: 2px solid;">
				...
				%>
					<tr>
						...
						<td><%=b.getBseats() %></td>
						<td><%=b.getBstatus()%></td>
						<td><a href="updateB?bid=<%=b.getBid()%>&bseats=<%=b.getBseats()%>&bstatus=<%=b.getBstatus() %>"><span class="badge badge-info">Update</span></a></td>
						<td><a href="deleteB?bid=<%=b.getBid()%>"><span class="badge badge-danger">Delete</span></a></td>
					</tr>
				<% } %>
				</tbody>
			</table>
