			...
			<table class="table" style="text-align: center;font-size: 5rem; border: 2px solid; border-bottom: 2px solid;">
				...
				%>
					<tr>
						...
						<td><%=d.getDstatus() %></td>
						<td>
							<a href="updateD?did=<%=d.getDid()%>&dname=<%=d.getDname()%>&daddress=<%=d.getDaddress()%>&dmob=<%=d.getDmob() %>&demail=<%=d.getDemail()%>&dgender=<%=d.getDgender() %>&dstatus=<%=d.getDstatus() %>">
								<span class="badge badge-info">Update</span>
							</a>
						</td>
						<td>
							<a href="delete?did=<%=d.getDid() %>">
